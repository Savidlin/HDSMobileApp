...
namespace HDSMobileApp.Helpers
{
    ...
    internal static class Helper
    {
            ...
            string methodName, string[] paramterNames, params object[] parameters)
        {
            ...
            {
                LogException(logger, methodName, e);
                throw new WebFaultException<String>(e.Message,
        ...
        internal static void CheckSearchCriteria(SearchCriteria criteria)
        {
            if (criteria.PageNumber < 0 || (criteria.PageNumber > 0 && criteria.PageSize <= 0))
            {
                throw new ArgumentException("pageNumber or pageSize not valid");
            }
        }


        /// <summary>
        /// Get real page number by page number incriteria an total records
        /// </summary>
        /// <param name="pageNumber">the criteria page number</param>
        /// <param name="totalRecords">the total records</param>
        /// <returns>the real page number</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static int GetPageNumber(int pageNumber, int totalRecords)
        {
            return pageNumber <= 0
                       ? ((totalRecords > 0) ? 1 : 0)
                       : pageNumber;
        ...
        /// <summary>
        /// Get real total pages by page number,page size in criteria and total records
        /// </summary>
        /// <param name="pageNumber">the criteria page number</param>
        /// <param name="pageSize">the criteria page size</param>
        /// <param name="totalRecords">the total records</param>
        /// <returns>the real total pages</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static int GetTotalPages(int pageNumber, int pageSize, int totalRecords)
        {
            return (pageNumber <= 0 || pageSize <= 0) ? ((totalRecords > 0) ? 1 : 0)
