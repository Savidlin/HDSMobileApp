...
class Defer {
     ...
     */
    static newDefer<T extends Q.Deferred<any>>(): T {
        return <T>Q.defer<any>();
    }


    static when<T>(promises: Q.IPromise<T>[]): Q.Promise<T[]> {
     ...
     */
    static runActionForAll<T, R>(args: T[], actionFunc: (def: Q.Deferred<R>, obj: T) => void): Q.Promise<R[]> {
     ...
     * @return {Promise} a promise that returns an array of all of the results returned from the calls to {@code actionFunc}
     */
    static runActionForAllInSeries<T, R>(args: T[], actionFunc: (def: Q.Deferred<R>, obj: T) => void, stopOnFirstError: boolean = false): Q.Promise<R[]> {
        if (typeof actionFunc !== "function") {
            throw new Error("incorrect arguments (" + args + "," + actionFunc + "), expected (Array, Function)");
        }
     ...
     */
    static chainTo<T>(srcPromise: Q.IPromise<T>, dstDfd: Q.Deferred<T>) {
     ...
     */
    static chainToWith<T, R>(srcPromise: Q.IPromise<T>, dstDfd: Q.Deferred<T>, successCall: (obj: T) => T, failureCall: (err) => R) {
