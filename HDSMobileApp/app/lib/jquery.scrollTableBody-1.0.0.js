(function ($) {
    ...
    function wrapTable(table, options) {
        ...
        var headerTable = $('<table id="headerTable' + id + '" style="table-layout:fixed;width:auto;margin-bottom:0;" class="jqstb-header-table ' + existingClasses + '"><thead><tr><td></td></tr></thead></table>'),
            footerTable = $('<table id="footerTable' + id + '"style="table-layout:fixed;width:auto;margin-bottom:' + existingMarginBottom + ';" class="jqstb-footer-table ' + existingClasses + '"><tfoot><tr><td></td></tr></tfoot></table>'),
            scrollDiv = '<div id="scroller' + id + '" class="jqstb-scroll" style="height:' + tableHeight + 'px;overflow-y:auto"></div>';
    ...
    function alignColumns(table) {
        table.each(function (index) {
            ...
            var $dataTable = $(this);
            var $headerTable = $('#headerTable' + id);
            var $footerTable = $('#footerTable' + id);

            // Place main table data inside of relevant scrollable div (using jQuery eq() and index)
            var $scrollDiv = $('#scroller' + id);
